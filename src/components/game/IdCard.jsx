...
export const IdCard = forwardRef(({ results, username = "Résistant Anonyme", theme = "retro" }, ref) => {
  ...
  const themeStyles = {
    retro: {
      container: "bg-black border-4 border-cyan-500 shadow-[0_0_30px_rgba(6,182,212,0.4)]",
      bgTexture: "bg-[linear-gradient(45deg,rgba(6,182,212,0.1)_1px,transparent_1px),linear-gradient(-45deg,rgba(234,179,8,0.1)_1px,transparent_1px)] bg-[size:20px_20px]",
      ...
      fontMono: "font-mono",
      fontSans: "font-sans",
      badge: "border-cyan-500 text-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]",
    },
    dark: {
      container: "bg-zinc-900 border border-green-500/50 shadow-2xl",
      bgTexture: "bg-[radial-gradient(circle_at_center,rgba(34,197,94,0.1)_0%,transparent_70%)]",
    ...
    >
      ...
      <div className="w-[220px] p-6 flex flex-col justify-between relative z-10 border-r border-white/10 bg-black/5">
        <div className="space-y-4">
          ...
          <div className={clsx(
            ...
            theme === 'retro' ? 'border-cyan-500' : theme === 'dark' ? 'border-green-500' : 'border-blue-200'
          )}>
            <Icons.Penguin className={clsx("w-14 h-14", t.textPrimary)} />
          </div>
          
          <div className="text-center">
            <div className={clsx("text-[10px] uppercase tracking-widest mb-1", t.textMuted)}>Agent</div>
            <h2 className={clsx("text-xl font-bold truncate px-2", t.textSecondary, t.fontSans)}>
              {username}
            </h2>
          </div>
        </div>
        ...
        <div className="text-center space-y-2">
          <div className={clsx("inline-block px-3 py-1 rounded border text-xs font-bold tracking-wider", t.badge)}>
            {getRank(global)}
          </div>
          <div className={clsx("text-4xl font-black", t.textPrimary, t.fontMono)}>
            {global}%
          </div>
      ...
      <div className="flex-1 p-6 flex flex-col relative z-10">
        <div className="flex justify-between items-start mb-4">
          <h3 className={clsx("text-sm uppercase tracking-widest font-bold", t.textMuted)}>
            Profil de Résistance
          </h3>
          <Icons.Shield className={clsx("w-6 h-6 opacity-50", t.textPrimary)} />
        </div>

        <div className="flex-1 flex items-center justify-center -ml-4">
          <CategoryRadar scores={categories} theme={theme} />
        </div>
