...
export default function CursorChallenge({ onUnlock }) {
  ...
  // Handle Input
  useEffect(() => {
    const handleKeyDown = (e) => setKeys((k) => ({ ...k, [e.key.toLowerCase()]: true, [e.code]: true }));
    const handleKeyUp = (e) => setKeys((k) => ({ ...k, [e.key.toLowerCase()]: false, [e.code]: false }));
    ...
    >
      ...
      <div 
        ...
        className={`relative w-[150px] h-[150px] rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
          progress > 0 ? "border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.4)]" : "border-yellow-500/30 border-dashed animate-spin-slow"
        }`}
      >
        {/* Progress Ring */}
      ...
      >
        ...
        >
          <path d="M12 2L2 22L12 18L22 22L12 2Z" fill="rgba(6,182,212,0.1)" />
        </svg>
      </motion.div>
