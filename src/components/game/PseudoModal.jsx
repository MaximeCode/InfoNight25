  ...
  >
            ...
            <div className="relative z-10 text-center space-y-6">
              <div className="space-y-2">
                <motion.h2 
                  className={`text-3xl font-bold font-sans ${isHackerMode ? "text-nird-neon glitch-text" : "text-white"}`}
                  animate={isHackerMode ? { textShadow: ["0 0 0px #06B6D4", "0 0 10px #06B6D4", "0 0 0px #06B6D4"] } : {}}
                >
                  {isHackerMode ? "SYSTEM OVERRIDE" : "Identité Requise"}
                </motion.h2>
                <p className={`text-sm ${isHackerMode ? "text-nird-neon/80 font-mono" : "text-nird-light/60"}`}>
                  {isHackerMode ? "> ACCESSING HIDDEN MODULE..." : "Pour générer votre carte de membre."}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="relative group">
                  ...
                  <div className={`absolute inset-0 rounded-lg opacity-0 group-focus-within:opacity-100 transition-opacity duration-300 pointer-events-none ${
                    isHackerMode ? "shadow-[0_0_20px_var(--color-nird-neon)]" : "shadow-[0_0_20px_var(--color-nird-green)]"
                  }`} />
                </div>
