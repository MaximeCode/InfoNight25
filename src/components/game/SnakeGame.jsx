...
const getRandomPos = (snake, existingItems = []) => {
  ...
  while (attempts < 100) {
    ...
    const onItem = existingItems.some((item) => item.x === newPos.x && item.y === newPos.y);
...
export default function SnakeGame({ onClose }) {
  ...
  // --- Game Loop ---
  const runGameTick = useCallback((timestamp) => {
    ...
    if (timestamp - lastMoveTimeRef.current >= speedRef.current) {
      ...
      if (newHead.x === foodRef.current.x && newHead.y === foodRef.current.y) {
        ...
        spawnMalus();
      } else if (malusRef.current && newHead.x === malusRef.current.x && newHead.y === malusRef.current.y) {
    ...
    >
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-black/80 border-4 border-cyan-500 rounded-xl shadow-[0_0_50px_rgba(6,182,212,0.5)] backdrop-blur-md overflow-hidden flex flex-col">
        ...
        <div className="flex justify-between items-center p-4 border-b border-cyan-500/30 bg-black/60 text-cyan-500 z-10">
          <Button variant="ghost" size="sm" onClick={onClose} className="text-red-500 hover:bg-red-500/10">
            <X className="w-5 h-5 mr-2" /> QUIT
          </Button>
          
          <div className="flex flex-col items-center">
            <span className="text-[10px] uppercase tracking-widest opacity-70">High Score</span>
        ...
        <div className="relative aspect-square max-h-[70vh] m-4 border border-white/10 rounded-lg overflow-hidden bg-black/50">
            ...
            return (
              <div
                ...
                style={{
                  ...
                  boxShadow: isHead ? '0 0 10px rgba(103, 232, 249, 0.6)' : '0 0 5px rgba(6, 182, 212, 0.3)',
                  zIndex: isHead ? 10 : 5
                }}
              >
                {/* Eyes on head */}
          ...
          >
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              <svg viewBox="0 0 24 24" className="w-4/5 h-4/5" style={{ filter: 'drop-shadow(0 0 4px rgba(34, 197, 94, 0.6))' }}>
                {/* Body */}
                <ellipse cx="12" cy="14" rx="7" ry="8" fill="#000000" />
                {/* Belly */}
                <ellipse cx="12" cy="15" rx="4.5" ry="6" fill="#FFFFFF" />
                {/* Left wing */}
                <ellipse cx="6" cy="13" rx="2" ry="4" fill="#000000" transform="rotate(-20 6 13)" />
                {/* Right wing */}
                <ellipse cx="18" cy="13" rx="2" ry="4" fill="#000000" transform="rotate(20 18 13)" />
                {/* Head */}
                <circle cx="12" cy="8" r="4" fill="#000000" />
                {/* Left eye */}
                <ellipse cx="10.5" cy="7.5" rx="1.2" ry="1.5" fill="#FFFFFF" />
                <circle cx="10.7" cy="7.8" r="0.6" fill="#000000" />
                {/* Right eye */}
                <ellipse cx="13.5" cy="7.5" rx="1.2" ry="1.5" fill="#FFFFFF" />
                <circle cx="13.3" cy="7.8" r="0.6" fill="#000000" />
                {/* Beak */}
                <ellipse cx="12" cy="9.5" rx="1" ry="1.2" fill="#FFA500" />
                {/* Feet */}
                <ellipse cx="10" cy="22" rx="1.5" ry="0.8" fill="#FFA500" />
                <ellipse cx="14" cy="22" rx="1.5" ry="0.8" fill="#FFA500" />
              </svg>
          ...
          <AnimatePresence>
            {gameState === "IDLE" && (
              ...
              >
                <h2 className="text-4xl font-black text-cyan-500 mb-4 tracking-tighter">SNAKE.EXE</h2>
                <p className="text-gray-400 mb-8 font-mono text-sm">USE ARROW KEYS TO MOVE</p>
                <Button onClick={startGame} variant="primary" size="lg" className="text-xl px-8 py-6 animate-pulse">
            ...
            {gameState === "GAME_OVER" && (
              ...
              >
                <h2 className="text-5xl font-black text-red-500 mb-2">CRITICAL ERROR</h2>
                <p className="text-gray-300 mb-8">SYSTEM CRASHED. SCORE: {score}</p>
                <div className="flex gap-4">
                  <Button onClick={startGame} variant="primary" className="bg-cyan-500 text-black border-none hover:bg-cyan-400">
                    <RefreshCw className="w-5 h-5 mr-2" /> REBOOT
                  </Button>
                  <Button onClick={onClose} variant="outline" className="border-red-500 text-red-500 hover:bg-red-500/10">
                    EXIT
                  </Button>
                </div>
              </motion.div>
            ...
            {gameState === "PAUSED" && (
              ...
              >
                <h2 className="text-3xl font-bold text-white mb-4">SYSTEM PAUSED</h2>
